...
CREATE TABLE IF NOT EXISTS public.quizzes (
  ...
  creator_name    TEXT,               -- for guest-created quizzes
  total_plays     INT DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS public.attempts (
  ...
  player_name     TEXT NOT NULL,       -- always store name for display
...
CREATE TABLE IF NOT EXISTS public.ad_settings (
  id                      INT PRIMARY KEY DEFAULT 1,  -- only ever 1 row
...
CREATE POLICY "quizzes_insert_auth"   ON public.quizzes FOR INSERT WITH CHECK (TRUE); -- allow guest creation
